...
fun getSumWithIndexDivisibleByThree(list: List<Long>): Long = list.withIndex()
    .filter { numbers -> numbers.index % 3 == 0 }
    .sumOf { numbersFiltered -> numbersFiltered.value }
...
fun Shop.getMostPopularProductInCity(): Map<City, Product> = customers
    .groupBy ({ it.city},
    {it.orders.map{ order -> order.products }
        .flatten()})
    .mapValues { listProducts -> listProducts.value
        .flatten()
        .groupingBy { it }
        .eachCount()
...
fun Shop.getProductsOrderedByAll(): Set<Product> = customers
    .map { it.orders.map { it.products }.flatten().toSet()}
    .reduce{acc, str -> acc.intersect(str)}
